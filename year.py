...
for ticker in tqdm(kospi_200_tickers, desc="Processing KOSPI 200 Tickers"):
    ...
    try:
        ...
        # income_statement.columns = ['Date', 'Net Income']
        # income_statement = income_statement.sort_values(by='Date')
        
        # 최근 5년간의 데이터만 선택
        recent_5_years = income_statement.tail(5)
        
        # 순이익 데이터 추출
        net_incomes = recent_5_years['Net Income'].apply(pd.to_numeric, errors='coerce').fillna(0).values
        
        # 데이터를 확인하기 위해 출력
        print(f"Ticker: {ticker}")
        print(recent_5_years)
